# This code is developed by Young Suh Lee (Sogang University, Korea)

# History:
# 21-03-24 Created
# 21-03-25 Converted to python file
...
class image_analysis:
    ...
    def measure_droplet(self):
        ...
        self.df_droplet = pd.DataFrame(self.droplet_list,
                                       columns=["path", "#droplet", "x", "y", "radius", "R", "G", "B", "H", "S", "V",
                                                "gray"])
    ...
    def save_droplet_data(self, csv=False, sql=False):
        if csv == True:
            self.df_droplet.to_csv("Droplet.csv", sep=',', index=False,
                                   header=["path", "#droplet", "x", "y", "radius", "R", "G", "B", "H", "S", "V",
                                           "gray"])
            self.df_concent.to_csv("Concentration.csv", sep=',', index=False,
                                   header=["threshold", "positive", "negative", "portion_pos", "conc_lower",
                                           "conc_upper", "conc_exact"])
        if sql == True:
            engine = create_engine('sqlite:///Output0.db', echo=False)
